...
- 能新增、编辑、删除收入或投放记录
- 能切换收入结算状态
- `Home` 的账号一览能正常切换账号并完成提交
- `Home` / `Archive` 的显式排序模式能进入和退出
...
- 新封面保存前需要先点击“确定封面位置”
- 封面上传后页面能正常展示
- 首页账号卡里的封面展示不应出现过度放大感

### 8.4 线上 smoke 建议

如果是直接发生产，建议优先跑“非破坏性”烟测，而不是一上来在线上写测试数据。

推荐顺序：

1. 首页加载、账号一览、排序模式进入/退出
2. 归档页日期切换、排序模式进入/退出
...
- 当前仓库还没有单独的 scripted production smoke suite
- 最近一次手工线上 smoke 已在 `2026-03-12` 跑过并通过
- 若要在线上验证真实写操作，优先使用可清理的测试数据前缀
